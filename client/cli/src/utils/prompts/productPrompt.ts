...
const createMessageFromProduct = (product: any) => {
	return `${product.brand || 'Migros'} ${product.name} (${product.quantity || product.noPriceReason})` + (product.price.value ? ` | ${product.price.value || product.noPriceReason} CHF` : '') + (product.price.unitPrice ? ` (${product.price.unitPrice.value} CHF pro ${product.price.unitPrice.unit})` : '')
}
